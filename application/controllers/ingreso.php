...
//Crear: Ingreso
    ...
    function consultar_pdf($id_ingreso, $salida_pdf) {
        ...
        $ingreso = $this->select_model->ingreso_prefijo_id($prefijo, $id);
        if ($ingreso == TRUE) {
            $responsable = $this->select_model->empleado($ingreso->id_responsable, $ingreso->dni_responsable);
...
// Este método tiene varias opciones, consulta la documentación para más información.
            ...
            $html .= '<table width="100%">'
                    ...
                    . '</tr>'
                    . '<tr>'
                    . '<td class="c1 c24 a2 c28" rowspan="3">COMPROBANTE DE INGRESO</td>'
                    . '<td class="c5 c23 c25 c26  c27 c28"><b>Número:</b></td><td class="c6 c23 c25 c26  c27 c28">' . $id_ingreso_limpio . '</td>'
                    . '</tr>'
                    . '<tr>'
                    . '<td class="c5 c23 c25 c26  c27 c28"><b>Fecha de emisión:</b></td><td class="c6 c23 c25 c26  c27 c28">' . date("Y-m-d", strtotime($ingreso->fecha_trans)) . '</td>'
                    . '</tr>'
                    . '<tr>'
...
// Este método tiene varias opciones, consulte la documentación para más información.
    ...
    function anular() {
        ...
        $data['sede'] = $this->select_model->sede_activa_responsable($_SESSION["idResponsable"], $_SESSION["dniResponsable"]);
        $data['action_validar'] = base_url() . "ingreso/validar_anular";
        $data['action_crear'] = base_url() . "ingreso/insertar_anular";
        $data['action_recargar'] = base_url() . "ingreso/anular";
        $data['action_validar_transaccion_anular'] = base_url() . "ingreso/validar_transaccion_anular";
        $this->parser->parse('ingreso/anular', $data);
        $this->load->view('footer');
    }
    ...
    function insertar_anular() {
        if ($this->input->post('submit')) {
            ...
            if (isset($error)) {
                ...
                $error1 = $this->update_model->ingreso_vigente($prefijo, $id, $vigente);
                if (isset($error1)) {
                    $data['trans_error'] = $error1 . "<p>Comuníque éste error al departamento de sistemas.</p>";
                    $this->parser->parse('trans_error', $data);
                } else {
    ...
    public function validar_transaccion_anular() {
        if ($this->input->is_ajax_request()) {
            ...
            $ingreso = $this->ingresom->ingreso_prefijo_id($prefijo, $id);
            if ($ingreso == TRUE) {
                if ($ingreso->vigente == 1) {
                    $response = array(
                        'respuesta' => 'OK',
                        'filasTabla' => ''
                    );
                    $response['filasTabla'] .= '<tr>
                            <td class="text-center">' . $ingreso->tipo . '</td>
                            <td class="text-center">$' . number_format($ingreso->total, 2, '.', ',') . '</td>
                            <td class="text-center">' . $ingreso->sede_caja . '-' . $ingreso->tipo_caja . '</td>
                            <td class="text-center">$' . number_format($ingreso->efectivo_ingresado, 2, '.', ',') . '</td>
                            <td class="text-center">' . $ingreso->cuenta_destino . '</td>
                            <td class="text-center">$' . number_format($ingreso->valor_consignado, 2, '.', ',') . '</td> 
                            <td class="text-center">' . $ingreso->responsable . '</td>                                
                            <td class="text-center">' . date("Y-m-d", strtotime($ingreso->fecha_trans)) . '</td>
                        </tr>';
                    ...
                    $response = array(
                        'respuesta' => 'error',
                        'mensaje' => '<p><strong><center>El ingreso, ya se encuentra anulado.</center></strong></p>'
                    );
                    echo json_encode($response);
                    return false;
                }
                ...
                $response = array(
                    'respuesta' => 'error',
                    'mensaje' => '<p><strong><center>El ingreso, no existe en la base de datos.</center></strong></p>'
                );
                echo json_encode($response);
                return false;
            }
