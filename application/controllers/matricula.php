...
//                $check_matricula_sede = TRUE; //Por el momento colocamos que puedan editar matriculas de cualquier sede.
    ...
    public function validar_plan_pagos() {
        ...
        if (($this->form_validation->run() == FALSE) || ($error_transaccion != "")) {
            ...
            redirect(base_url() . "matricula/consultar_pdf_plan_pagos/" . $id . "/I");
        }
    }

    function consultar_pdf_plan_pagos($id, $salida_pdf) {
        $this->load->model('matriculam');
...
// Este método tiene varias opciones, consulta la documentación para más información.
            ...
            $html .= 'p.b3{font-family: helvetica, sans-serif;font-size:12px;font-weight: bold;line-height:10px;text-align:center;}';
            ...
            $html .= '<tr>'
                    ...
                    . '<td class="d2 a2 c30 c27 c28">' . date("Y-m-d", strtotime($matricula->fecha_matricula)) . '</td>'
            ...
            for ($i = 1; $i <= $matricula->cant_cuotas; $i++) {
                ...
                $html .= '<tr>'
                        ...
                        . '<td class="d2 a2 c30 c27 c28">' . date("Y-m-d", strtotime("$matricula->fecha_matricula +$i month")) . '</td>'
            ...
            $html .= '<tr>'
                    ...
                    . '</tr>'
                    . '<tr>'
                    . '<td class="d10 c20 a2 c25 c26 c27 c28">Certifico que acepto éste cronograma de pagos.<br><br><br><br><br>______________________________________<br>Firma y documento titular</td>'
...
// Este método tiene varias opciones, consulte la documentación para más información.
    ...
    function consultar_pagos_matricula() {
        $data["tab"] = "consultar_pagos_matricula";
        $this->isLogin($data["tab"]);
        $this->load->view("header", $data);
        $data['error_consulta'] = "";
        $data['action_recargar'] = base_url() . "matricula/consultar_pagos_matricula";
        $data['action_valida_llena_pagos'] = base_url() . "matricula/valida_llena_pagos";
        $this->parser->parse('matricula/consultar_pagos_matricula', $data);
        $this->load->view('footer');
    }
    ...
    public function validar_pagos_matricula() {
        $this->escapar($_POST);
        $this->form_validation->set_rules('id', 'Número de Matrícula', 'required|trim|max_length[13]|integer|callback_valor_positivo');
        $id = $this->input->post('id');
        $error_transaccion = "";
    ...
    function consultar_pdf_pagos_matricula($id, $salida_pdf) {
        $this->load->model('matriculam');
        $matricula = $this->matriculam->matricula_id($id);
        if ($matricula == TRUE) {
            $dni_abreviado_ejecutivo = $this->select_model->t_dni_id($matricula->dni_ejecutivo)->abreviacion;
